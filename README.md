...
| autoIndent              | `null \| { enabled?: boolean; tuneNames?: string[]; }`                                                     | If provided will automatically indent new blocks to same level as the previous block. Provide a list of `tuneNames` for which it should be applied, applies to all by default.                                       | `false`      |
